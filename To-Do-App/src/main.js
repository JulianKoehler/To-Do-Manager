...
function changeTodoStatus(checkboxId, status) {
    ...
    todoItems.forEach(todo => {
        ...
        if (todo.id == checkboxId && todo.isDone === true) { // check if the IDs are matching AND if it's done
            const TODO_ITEM_ARR = todoItems.splice(todoItems.indexOf(todo), 1); // remove the todo from the todo list and save it in this variable (as a new array)
    ...
    todoItems.forEach(todo => {
        if (todo.id == updateButtonID) {
            ...
            if (UPDATED_TODO_DUEDATE != '') {
                ...
                todo.dueDate = 'No due Date - Keep it calm';
            };
...
function renderItems() {
    ...
    todoItems.forEach(todo => {
        ...
        if (todo.dueDate === '') {  // in case the user has not selected any date the program shall only show the todo title
            TEXT_AREA.innerHTML = `${todo.title}`;
        } else {
            TEXT_AREA.innerHTML = `${todo.title} - Finish by ${todo.dueDate}.`;
        }
...
function addItem() {
    ...
    if (dueDate != '') {
        ...
        dueDate = 'No due Date - Keep it calm'
    }
